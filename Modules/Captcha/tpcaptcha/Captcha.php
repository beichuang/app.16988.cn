...
{
    protected $config = [
        'seKey'    => 'zhangwan.app.img.captcha',
        // 验证码加密密钥
    ...
    public function check($code, $id = '')
    {
        ...
        if (time() - $secode['verify_time'] > $this->expire) {
            SessionHelper::set($key, '');
            return false;
        }

        if ($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && SessionHelper::set($key, '');
            return true;
        }
    ...
    public function entry($id = '')
    {
        ...
        SessionHelper::set($key . $id, $secode, '');
