...
class Http {
    ...
    private static function init() {
        if (function_exists('curl_init')) {
            ...
            throw new \SnsAuth\Exception\SnsOAuthException('服务器不支持CURL');
        }
    }
    ...
    private static function exec($ch) {
        ...
        if ($rsp !== false) {
            ...
            curl_close($ch);
            throw new \SnsAuth\Exception\SnsOAuthException("curl出错，$errorMsg", $errorCode);
        }
    }
    ...
    public static function postRawSsl($url, $raw, $options) {
        ...
        if (!array_key_exists('cert_path', $options) || !array_key_exists('key_path', $options) || !array_key_exists('ca_path', $options)) {
            throw new \SnsAuth\Exception\SnsOAuthException('证书文件路径不能为空');
        }
    ...
    public static function saveImage($url, $path, $filename = null) {
        ...
        if ($img !== false) {
            ...
            $errorMsg  = curl_error($ch);
            curl_close($ch);
            throw new \SnsAuth\Exception\SnsOAuthException("获取头像出错，$errorMsg", $errorCode);
        }
        $content_type = explode('/', $file_info['content_type']);
        if (strtolower($content_type[0]) != 'image') {
            throw new \SnsAuth\Exception\SnsOAuthException('下载地址文件不是图片');
        }
