...
class Weibo extends \SnsAuth\OAuth2\OAuth {
    ...
    protected function parseToken($result) {
        ...
        if ($data['access_token'] && $data['expires_in'] && $data['remind_in'] && $data['uid']) {
            ...
            throw new \SnsAuth\Exception\SnsOAuthException("获取新浪微博ACCESS_TOKEN出错：{$data['error']}");
        }
    }
    ...
    public function openid() {
        ...
        if (isset($data['openid']))
            ...
            throw new \SnsAuth\Exception\SnsOAuthException('没有获取到新浪微博用户ID！');
    }
    ...
    public function userinfo() {
        ...
        if (isset($rsp['error_code'])) {
            throw new \SnsAuth\Exception\SnsOAuthException('接口访问失败！' . $rsp['error']);
        } else {
            $userinfo = array(
                'openid'  => $this->openid(),
                'channel' => 'weibo',
