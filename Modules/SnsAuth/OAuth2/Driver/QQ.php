...
namespace SnsAuth\OAuth2\Driver;

use SnsAuth\helper\Http;

class QQ extends \SnsAuth\OAuth2\OAuth {
    ...
    protected function parseToken($result) {
        ...
        if ($data['access_token'] && $data['expires_in']) {
            ...
            throw new \SnsAuth\Exception\SnsOAuthException("获取腾讯QQ ACCESS_TOKEN 出错：{$result}");
        }
    }
    ...
    public function openid() {
        ...
        if (isset($data['openid'])) {
            ...
            if (isset($data['openid'])) {
                ...
                throw new \SnsAuth\Exception\SnsOAuthException("获取用户openid出错：{$data['error_description']}");
            }
        } else {
            throw new \SnsAuth\Exception\SnsOAuthException('没有获取到openid！');
        }
    }
    ...
    public function userinfo() {
        ...
        if (!$rsp || $rsp['ret'] != 0) {
            throw new \SnsAuth\Exception\SnsOAuthException('接口访问失败！' . $rsp['msg']);
        } else {
            $userinfo = array(
                'openid'  => $this->openid(),
                'channel' => 'qq',
