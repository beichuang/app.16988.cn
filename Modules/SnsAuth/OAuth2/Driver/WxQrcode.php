...
class WxQrcode extends \SnsAuth\OAuth2\OAuth {
    ...
    protected function parseToken($result) {
        ...
        if ($data['access_token'] && $data['expires_in'] && $data['openid']) {
            ...
            throw new \SnsAuth\Exception\SnsOAuthException("获取微信 ACCESS_TOKEN 出错：{$result}");
        }
    }
    ...
    public function openid() {
        ...
        if (isset($data['openid']))
            ...
            throw new \SnsAuth\Exception\SnsOAuthException('没有获取到微信用户ID！');
    }
    ...
    public function userinfo() {
        ...
        if (!$rsp || (isset($rsp['errcode']) && $rsp['errcode'] != 0)) {
            throw new \SnsAuth\Exception\SnsOAuthException('接口访问失败！' . $rsp['errmsg']);
        } else {
            $userinfo = array(
