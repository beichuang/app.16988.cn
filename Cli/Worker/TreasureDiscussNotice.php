...
class TreasureDiscussNotice
{
    ...
    public function __construct()
    {
        ...
        $this->queueFlag = config('data_change_notice_queue.interface.treasureDiscuss.appMessagePushQueue');
        $this->queueFrequency = config('data_change_notice_queue.interface.treasureDiscuss.frequency', 200000);
    }
    ...
    public function run()
    {
        $restartStart = config('data_change_notice_queue.interface.treasureDiscuss.workrRestart.0');
        $restartEnd = config('data_change_notice_queue.interface.treasureDiscuss.workrRestart.1');
        while (true) {
    ...
    private function appMessagePush($message)
    {
        ...
        $msgType = AppMessagePush::PUSH_TYPE_TREASURE_DISCUSS;
        $uid = $message['uid'];
        $title = $message['title'];
